...
def _metadata_is_consistent(metadata):
    ...
    # Convert map objects to lists for checking length consistency
    fields = list(metadata['fields'])
    counts = list(metadata['count'])
    types = list(metadata['type'])
    print(counts)
    # Check if lengths of 'type', 'count' and 'fields' are the same
    checks.append((lambda m: len(types) == len(counts) == len(fields),
                   f'length of type, count and fields must be equal,. got t {len(types) } c {len(counts) } f {len(fields)}'))
    
    # Check if 'height', 'width', and 'points' are positive
    ...
    # Check if 'data' type is one of the valid options
    ...
    # Perform all checks and report any failures
    ok = True
    for check, error_message in checks:
        if not check(metadata):
            print(f">> problem with metadata : {error_message}  -  {check}")
            ok = False
...
def _build_dtype(metadata):
    ...
    *TODO* allow 'proper' multi-count fields.
    ...
    print(f"the coutns {counts}")
    # Iterate over fields, counts, types, and sizes
    for f, c, t, s in zip(fields, counts, types, sizes):
        np_type = pcd_type_to_numpy_type.get((t, s), None)
        
        if np_type is None:
            raise ValueError(f"Unknown type mapping for ({t}, {s})")
        
        if c == 1:
            fieldnames.append(f)
            typenames.append(np_type)
        else:
            fieldnames.extend([f'{f}_{i:04d}' for i in range(c)])
